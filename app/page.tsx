...
const STATS = [
  ...
  { value: regencies.length, label: "wilayah" },
    ...
    <>
      <section className="relative isolate overflow-hidden">
        <div className="absolute inset-0 -z-10">
          <Image
            src="/images/destinasi/prambanan5.jpg"
            alt=""
            fill
            priority
            sizes="100vw"
        ...
        <div className="mx-auto max-w-page px-gutter py-section">
          ...
          <p className="mt-5 max-w-xl text-body-lg text-on-scrim-muted">
            {destinations.length} destinasi di {regencies.length} wilayah
      ...
      <section className="mx-auto max-w-page px-gutter py-block">
        ...
        <ul className="mt-6 flex flex-wrap gap-3">
          {categories.map((category) => (
            <li key={category.id}>
              <Link
                href={`/destinasi?kategori=${category.id}`}
